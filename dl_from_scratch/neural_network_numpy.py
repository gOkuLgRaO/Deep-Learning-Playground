...
class NeuralNetwork:
    def __init__(self, layer_sizes, activation="relu", output_activation="softmax",
                 loss="cross_entropy", optimizer="gd", lr=0.01, beta1=0.9, beta2=0.999, eps=1e-8):
        """
        layer_sizes: list of integers, e.g. [input_dim, hidden1, ..., output_dim]
        activation: hidden activation ("relu", "sigmoid", "tanh")
        output_activation: output activation ("softmax", "sigmoid")
        loss: "cross_entropy" or "mse"
        optimizer: "gd" (batch gradient descent), "sgd" (mini-batch), "adam"
        lr: learning rate
    ...
    def _init_weights(self):
        ...
        for i in range(1, len(self.layer_sizes)):
            params[f"W{i}"] = np.random.randn(self.layer_sizes[i], self.layer_sizes[i - 1]) * np.sqrt(
    ...
    def _activation_derivative(self, A, func):
        if func == "relu":
            ...
            raise ValueError("No derivative defined for softmax")
    ...
    def forward(self, X):
        ...
        Z = self.params[f"W{L}"] @ cache[f"A{L - 1}"] + self.params[f"b{L}"]
    ...
    def backward(self, cache, Y_true):
        ...
        for i in reversed(range(1, L)):
            dZ = dA_prev * self._activation_derivative(cache[f"A{i}"], self.activation_name)
